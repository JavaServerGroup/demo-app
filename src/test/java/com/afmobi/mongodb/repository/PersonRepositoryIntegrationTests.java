...
import com.afmobi.mongodb.repository.pojo.UsernameAndPassword;
...
public class PersonRepositoryIntegrationTests {
	...
	public void findsAllWithGivenIds() {
		Iterable<Person> result = repository.findAll(Arrays.asList(dave.getId(), boyd.getId()));
	...
	public void findsByFirstname() {
		...
		assertThat(result.get(0), is(leroi));
	}
	
	//用@Query查找，只返回指定的field
	@Test
	...
	public void bindsDateParameterForLessThanPredicateCorrectly() {
		List<Person> result = repository.findByCreatedAtLessThan(boyd.getCreatedAt());
	...
	public void bindsDateParameterForGreaterThanPredicateCorrectly() {
		List<Person> result = repository.findByCreatedAtGreaterThan(carter.getCreatedAt());
	...
	public void bindsDateParameterToBeforePredicateCorrectly() {
		List<Person> result = repository.findByCreatedAtBefore(boyd.getCreatedAt());
		assertThat(result.size(), is(3));
		assertThat(result, hasItems(dave, oliver, carter));
	}
	...
	public void bindsDateParameterForAfterPredicateCorrectly() {
		List<Person> result = repository.findByCreatedAtAfter(carter.getCreatedAt());
		assertThat(result.size(), is(4));
		assertThat(result, hasItems(boyd, stefan, leroi, alicia));
	}
	...
	public void bindsDateParameterForManuallyDefinedQueryCorrectly() {
		List<Person> result = repository.findByCreatedAtLessThanManually(boyd.getCreatedAt());
	...
	public void readsDocumentsWithNestedPolymorphismCorrectly() {
		...
		usernameAndPassword.setUsername("dave");
		usernameAndPassword.setPassword("btcs");

		dave.setCredentials(usernameAndPassword);
	...
	public void executesSingleEntityQueryWithProjectionCorrectly() {
		...
		assertThat(result.getLastname(), is("Beauford"));
	}
